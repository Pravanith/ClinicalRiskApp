        ...
        with st.expander("⚠️ Detailed Clinical Alerts & AI Assessment", expanded=True):
            ...
            if st.button("⚡ Generate AI Clinical Assessment"):
                with st.spinner("Consulting Medical AI..."):
                    ...
                    st.caption("⚠️ AI-Generated Insight. Verify with clinical protocols.")# --- MODULE 2: PATIENT HISTORY (SQL) ---
...
def render_dashboard():
    ...
    with c2:
        ...
        report_text = f"""
        CLINICAL RISK SUMMARY REPORT
        ----------------------------
        Patient ID: {data.get('id', 'Unknown')}
        Age: {data.get('age', 'N/A')}
        Status: {data.get('status', 'N/A')}
        
        RISK STRATIFICATION
        -------------------
        Bleeding Risk (XGBoost): {data.get('bleeding_risk', 0):.1f}%
        AKI Risk (KDIGO):        {data.get('aki_risk', 0)}%
        Sepsis Score (qSOFA):    {data.get('sepsis_risk', 0)}
        Hypoglycemia Alert:      {"YES" if data.get('hypo_risk', 0) > 0 else "NO"}
        
        GENERATED BY CLINICAL RISK MONITOR
