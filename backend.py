...
def init_db():
    ...
    # Auto-migration: Check if 'name' column exists, add if missing (for old DBs)
    try:
        c.execute("SELECT name FROM patient_history LIMIT 1")
    except sqlite3.OperationalError:
        try:
            c.execute("ALTER TABLE patient_history ADD COLUMN name TEXT")
        except:
            pass # Column might already exist or DB is locked
            
    conn.commit()
    conn.close()

def save_patient_to_db(name, age, gender, sbp, aki, bleed, status):
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute('''
            INSERT INTO patient_history (name, age, gender, sbp, aki_risk_score, bleeding_risk_score, status)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (name, age, gender, sbp, aki, bleed, status))
        conn.commit()
...
def calculate_sepsis_risk(sys_bp, resp_rate, altered_mental, temp_c):
    # This logic is mostly handled in app.py now via SIRS/qSOFA hybrid
    # Keeping generic fallback here for safety
    qsofa = 0
...
def calculate_sirs_score(temp_c, hr, resp_rate, wbc):
    score = 0
    # Note: Temp logic is handled in app.py due to Unit conversion (F/C)
    if hr > 90: score += 1
...
def consult_ai_doctor(role, user_input, patient_context=None):
    try:
        ...
        if role == 'risk_assessment':
            ...
            4. Cite relevant 2025 guidelines (e.g., KDIGO, Surviving Sepsis, ADA) where applicable.
            """
        else:
